...
def job1():#only to update the sql database
    print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} -Updating DB")
    daily_update(time.time)
    print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} -Generating json representation of DB")
    verify()
    
    
def job2():#only yields notifications
    print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} -Delivering Notifications")
    ...
    for prod_type,days_left in rows:
        ...
        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} -NOTIFICATION {i}")
        print(f'\n{notif}')
        i+=1
